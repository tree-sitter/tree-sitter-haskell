...
import TSH.Data.Options (BitmapOptions (..), BitmapPreset (..), Command (..), Options (..))
...
bitmapCommand = do
  preset <- optional (option presetValues (long "preset" <> short 'p' <> help presetHelp))
  ...
  pure BitmapOptions {preset, categories, limit, file, gapSize, unicodeData, name}
  where
