...
import TSH.Data.Options (BitmapOptions (BitmapOptions), BitmapPreset (..), presetSlug)
...
targetBlocks options allPoints = do
  ...
  targetPoints <- maybe noMatch pure (pointsInCategories targetCats options.limit allPoints)
  pure (maybe single pointBlocks options.gapSize targetPoints)
  where
    single points =
...
idBitmaps options points = do
  ...
  where
    name = fromMaybe "identifier" (options.name <|> (presetSlug <$> options.preset))
...
genBitmaps cwd options = do
  points <- readCodepoints cwd options.unicodeData
...
writeBitmaps cwd options = do
  ...
  writeOutput options (renderFile [bitmaps])
