...
import TSH.Data.Options (BitmapOptions, BitmapPreset (..), categories, name, preset)
import TSH.Data.Unicode (CatAbbr (..), Codepoint (..))
...
writeBitmapTest cwd baseOptions = do
  points <- readCodepoints cwd baseOptions.unicodeData
  bitmaps_id <- idBitmaps options_id points
  let test_id = genTest_id (limitItems baseOptions.limit points)
  writeOutput options_id (catLines (includes : renderBitmaps bitmaps_id ++ [test_id]))
  ...
  where
    options_id = baseOptions {preset = Just Id, categories = Nothing, name = Nothing}
